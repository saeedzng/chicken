...
send_buy_chicken_order,wallet_owner_address, wallet_referal_address,wallet_master_address,
  ...
  return (
    <div>
      ...
      {connected && (
       <><label>your referal address  </label><><input type="text" value={referal_address} onChange={(e) => setReferal_address(e.target.value)} /><br/>
              <br />
              <button className='button' onClick={() => { sendDeployByMaster(address(referal_address)); } }> Create Wallet Contract</button></></>
      )}
      {!connected && (
